...
public class FragmentDeviceMissionDetail extends Fragment implements OnChartGestureListener {
    MainActivity mActivity;
    View mViewRoot;
    private Unbinder unbinder;
    LineChart mChartPressure;
    LineChart mChartTemperature;
    @BindView(R.id.frg_mission_detail_tv_temperature)
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        mViewRoot = inflater.inflate(R.layout.fragment_device_mission_details, container, false);
        unbinder = ButterKnife.bind(this, mViewRoot);
        mChartPressure = (LineChart) mViewRoot.findViewById(R.id.mission_chart_pressure);
        mChartTemperature = (LineChart) mViewRoot.findViewById(R.id.mission_chart_temperature);
    ...
    /*Get database pressure and temperature */
...
//            System.out.println("UTC Millis * " + utcDate.getTime() + " ------  " + dateFormatLocal.format(utcDate));
    ...
    /*Init Pressure graph data*/
    private void initPressureData() {
        // no description text
        mChartPressure.getDescription().setEnabled(false);
        mChartPressure.setOnChartGestureListener(this);
        // enable touch gestures
        mChartPressure.setTouchEnabled(true);
        mChartPressure.setPinchZoom(true);
        mChartPressure.setBorderColor(Color.WHITE);
        mChartPressure.setDrawBorders(true);
        // enable scaling and dragging
        mChartPressure.setDragEnabled(true);
        mChartPressure.setScaleEnabled(true);
        mChartPressure.setDrawGridBackground(false);
        mChartPressure.setHighlightPerDragEnabled(false);

        // set an alternative background color
        mChartPressure.setBackgroundColor(Color.TRANSPARENT);
        mChartPressure.setExtraOffsets(0, 0, 0, 0);
        mChartPressure.getContentRect().set(0, 0, mChartPressure.getWidth(), mChartPressure.getHeight());
        mChartPressure.setViewPortOffsets(getResources().getDimension(R.dimen._25sdp), getResources().getDimension(R.dimen._25sdp), getResources().getDimension(R.dimen._10sdp), 0f);
        ...
        XAxis xAxis = mChartPressure.getXAxis();
...
//        });
        ...
        YAxis leftAxis = mChartPressure.getAxisLeft();
...
//        leftAxis.setGranularity(1f);
        YAxis rightAxis = mChartPressure.getAxisRight();
        rightAxis.setEnabled(false);
        mChartPressure.getAxisRight().setDrawGridLines(true);
        mChartPressure.getAxisLeft().setDrawGridLines(true);
        mChartPressure.getXAxis().setDrawGridLines(true);
        mChartPressure.setNoDataText("");
//        mChartPressure.setVisibleXRangeMaximum(1000f);
        if (mArrayListPresTemp.size() < 10000 && mArrayListPresTempDive2.size() < 10000) {
            mChartPressure.animateX(500);
        } else {
    ...
    private void initTemperatureData() {
        ...
        mChartTemperature.setViewPortOffsets(getResources().getDimension(R.dimen._25sdp), 0f, getResources().getDimension(R.dimen._10sdp), getResources().getDimension(R.dimen._18sdp));
...
//        mChartTemperature.setVisibleXRangeMaximum(1000f);
        if (mArrayListPresTemp.size() < 10000 && mArrayListPresTempDive2.size() < 10000) {
...
//        System.out.println("JD-mIntStationaryIntervalDive2-" + mIntStationaryIntervalDive2);
        int i = 0;
        Entry mEntry;
        if (mArrayListPresTemp.size() > 0) {
            ...
            mv.setChartView(mChartPressure); // For bounds control
            mChartPressure.setMarker(mv);
            mChartPressure.invalidate();
            i = 0;
            for (int x = 0; x < mArrayListPresTemp.size(); x++) {
                try {
                    mFloatPressure = Float.parseFloat(mArrayListPresTemp.get(x).getPressure_depth());
                    mFloatPressure = Float.parseFloat(String.format("%.2f", mFloatPressure));
//                    System.out.println("JD-mFloatPressureDepth-" + mFloatPressure);
                    mEntry = new Entry((float) i, mFloatPressure);
                    values.add(mEntry);
        ...
        if (!isSingleDataSelect && isDiveDataSelect) {
            if (mArrayListPresTempDive2.size() > 0) {
                ...
                for (int x = 0; x < mArrayListPresTempDive2.size(); x++) {
                    try {
                        mFloatPressure = Float.parseFloat(mArrayListPresTempDive2.get(x).getPressure_depth());
                        mFloatPressure = Float.parseFloat(String.format("%.2f", mFloatPressure));
//                        System.out.println("JD-mFloatPressureDepth-" + mFloatPressure);
                        mEntry = new Entry((float) i, mFloatPressure);
        ...
        if (!isSingleDataSelect && isDiveDataSelect) {
            ...
            if (mChartPressure.getData() != null &&
                    mChartPressure.getData().getDataSetCount() > 0) {
                set1 = (LineDataSet) mChartPressure.getData().getDataSetByIndex(0);
                set2 = (LineDataSet) mChartPressure.getData().getDataSetByIndex(1);
                set1.setValues(values);
                set2.setValues(valuesDive2);
                mChartPressure.getData().notifyDataChanged();
                ...
                set1.setCircleColor(getResources().getColor(R.color.dive_one));
                ...
                set2.setCircleColor(getResources().getColor(R.color.dive_two));
        ...
        Legend l = mChartPressure.getLegend();
        l.setEnabled(false);
    }
...
// un-highlight values after the gesture is finished and no single-tap
        if (lastPerformedGesture != ChartTouchListener.ChartGesture.SINGLE_TAP)
            mChartTemperature.highlightValues(null); // or highlightTouch(null) for callback to onNothingSelected(...)
    }
