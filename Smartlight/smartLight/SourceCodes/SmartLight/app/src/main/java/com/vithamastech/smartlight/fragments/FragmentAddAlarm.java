...
public class FragmentAddAlarm extends Fragment {
    String TAG = FragmentAddAlarm.class.getSimpleName();
    ...
    int mAlarmColor = Color.argb(255, 255, 255, 255);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        mActivity.mTextViewTitle.setText(R.string.frg_alarm_add_title);
        ...
        mActivity.setOnDevicesStatusChange(new onDeviceConnectionStatusChange() {
            ...
            public void addScanDevices(VoBluetoothDevices mVoBluetoothDevices) {
                if (mVoBluetoothDevices.getDeviceHexData().substring(32, 36).toLowerCase().equals(URLCLASS.ALARM_ADD_RSP)) {
        ...
        mActivity.mTextViewAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (isAdded()) {
                    ...
                    String maxAlarmCount = mActivity.mDbHelper.getQueryResult("select count(*) from " + mActivity.mDbHelper.mTableAlarm + " where " + mActivity.mDbHelper.mFieldAlarmIsActive + "= '1'" + " AND " + mActivity.mDbHelper.mFieldAlarmUserId + "= '" + mActivity.mPreferenceHelper.getUserId() + "'");
                    try {
                        int mAlarmLimit = Integer.parseInt(maxAlarmCount);
                        if (mAlarmLimit < 6) {
                            DataHolder mDataHolder;
                            try {
                                String url = "select * from " + mActivity.mDbHelper.mTableAlarm + " where " + mActivity.mDbHelper.mFieldAlarmIsActive + "= '0'" + " AND " + mActivity.mDbHelper.mFieldAlarmUserId + "= '" + mActivity.mPreferenceHelper.getUserId() + "'" + " Limit 1";
                                mDataHolder = mActivity.mDbHelper.read(url);
                                if (mDataHolder != null) {
                                    VoAlarm mVoAlarm = null;
                                    for (int i = 0; i < mDataHolder.get_Listholder().size(); i++) {
                                        System.out.println("mFieldAlarmLocalID=" + mDataHolder.get_Listholder().get(i).get(mActivity.mDbHelper.mFieldAlarmLocalID));
                                    ...
                                    mIntMaxAlarmCount = Integer.parseInt(mVoAlarm.getAlarm_count_no());
                                    mStringAlarmLocalID = mVoAlarm.getAlarm_local_id();
                                    ...
                                    if (mStringSelectedDays != null && !mStringSelectedDays.equalsIgnoreCase("") && !mStringSelectedDays.equalsIgnoreCase("null")) {
                                        if (mArrayListCheckedDevice.size() > 0) {
                                            if (mActivity.isDevicesConnected || mActivity.getIsDeviceSupportedAdvertisment()) {
                                                mActivity.showProgress(getResources().getString(R.string.frg_adding_alarm), true);
...
//        String subStringSuccess;
        ...
        if (isAnyDeviceAddedInAlarm) {
            ...
            mContentValues.put(mActivity.mDbHelper.mFieldAlarmCountNo, mIntMaxAlarmCount);
            mContentValues.put(mActivity.mDbHelper.mFieldAlarmIsActive, "1");
            mContentValues.put(mActivity.mDbHelper.mFieldAlarmCreatedAt, mDateFormatDb.format(currentLocalTime));
            ...
            String[] mArray = new String[]{mStringAlarmLocalID};
            ...
            String mStringQuery = "delete from " + mActivity.mDbHelper.mTableAlarmDeviceList + " where " + mActivity.mDbHelper.mFieldADAlarmLocalID + "= '" + mStringAlarmLocalID + "'";
            mActivity.mDbHelper.exeQuery(mStringQuery);

            ContentValues mContentValuesAD;
            for (int i = 0; i < mArrayListCheckedDevice.size(); i++) {
                if (mArrayListCheckedDevice.get(i).getIsDeviceSyncWithGroup()) {
                    ...
                    mContentValuesAD.put(mActivity.mDbHelper.mFieldADAlarmLocalID, mStringAlarmLocalID);
                    mContentValuesAD.put(mActivity.mDbHelper.mFieldADAlarmServerID, "");
                    mContentValuesAD.put(mActivity.mDbHelper.mFieldADDeviceLocalID, mArrayListCheckedDevice.get(i).getDevicLocalId());
                    mContentValuesAD.put(mActivity.mDbHelper.mFieldADDeviceServerID, mArrayListCheckedDevice.get(i).getDeviceServerId());
                    mContentValuesAD.put(mActivity.mDbHelper.mFieldADDeviceStatus, mArrayListCheckedDevice.get(i).getDevice_is_active());
    ...
    private void showAlarmAddAlert() {
        mActivity.isAddDeviceScan = false;
        mActivity.mUtility.errorDialogWithCallBack(getResources().getString(R.string.frg_routine_add_success), 0, false, new onAlertDialogCallBack() {
            @Override
            public void PositiveMethod(DialogInterface dialog, int id) {
    ...
    private void getDBDeviceList() {
        ...
        if (mDeviceListAdapter == null) {
            ...
            try {
                String url = "select * from " + mActivity.mDbHelper.mTableDevice + " where " + mActivity.mDbHelper.mFieldDeviceIsActive + "= '1'" + " AND " + mActivity.mDbHelper.mFieldDeviceUserId + "= '" + mActivity.mPreferenceHelper.getUserId() + "'";
