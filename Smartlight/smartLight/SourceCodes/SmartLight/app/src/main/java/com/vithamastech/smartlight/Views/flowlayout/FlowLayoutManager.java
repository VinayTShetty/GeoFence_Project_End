...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
	...
	@Override
	public RecyclerView.LayoutParams generateLayoutParams(Context c,
			AttributeSet attrs) {
	...
	public void onLayoutChildren(RecyclerView.Recycler recycler,
		...
		for (int i = 0; i < count; i++) {
			...
			view = new ViewDefinition(this.config, child);
		...
		this.config.setMaxWidth(this.getWidth() - this.getPaddingRight()
				- this.getPaddingLeft());
		this.config.setMaxHeight(this.getHeight() - this.getPaddingTop()
	...
	private void applyPositionsToViews(LineDefinition line) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			layoutDecorated(
					...
					this.getPaddingLeft() + line.getLineStartLength()
							+ child.getInlineStartLength() + child.getWidth(),
					this.getPaddingTop() + line.getLineStartThickness()
							+ child.getInlineStartThickness()
							+ child.getHeight());
		}
